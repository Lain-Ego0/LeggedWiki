...
$$\text{if } \|r_k\| > \mu_{\text{ratio}} \|s_k\| : \quad \rho \leftarrow \tau \rho \quad \text{（增大 }\rho\text{，加速原可行性收敛）}$$
$$\text{if } \|s_k\| > \mu_{\text{ratio}} \|r_k\| : \quad \rho \leftarrow \rho/\tau \quad \text{（减小 }\rho\text{，加速对偶可行性收敛）}$$
...
$$x_i^{k+1} = \text{prox}_{f_i/\rho}(z^k - u_i^k) \quad \text{（}N\text{ 个子问题并行求解）}$$
